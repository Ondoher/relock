...
function assignConfig(loaded) {
    Object.assign(config, {
        ...
        packageLockedFilename: 'package-locked.json',
...
function flattenTree(tree) {
    ...
    function buildIndexes(node, path) {
        node.dependencies.forEach(function(dependency) {
            ...
            moduleIndex[dependency.name] = moduleIndex[dependency.name] || {};
            moduleIndex[dependency.name][name] = moduleIndex[dependency.name][name] || [];
            moduleIndex[dependency.name][name].push(path.join('|'));
    ...
    function findVariant(module) {
        var keys = Object.keys(module);
        var shortestVariant;
        var pathLength = Number.MAX_SAFE_INTEGER;

        keys.forEach(function(key) {
            var variant = module[key];
            variant.forEach(function(path) {
    ...
    function hoistOne(module) {
        var variant = findVariant(module);
        var found = module[variant];
        found.forEach(function(path) {
            add(path, variant)
        });

    // already did this, remove it
    ...
    function hoist() {
        ...
        var keys = Object.keys(moduleIndex);
        var module;
        ...
        while (keys.length) {
            keys.forEach(function(key) {
                ...
                if (Object.keys(module).length === 0) {
                    delete moduleIndex[key];
                }
            });

            keys = Object.keys(moduleIndex);
        }
    }
...
function handleFirstTime() {
    ...
    curPackage.requires = curPackage.dependencies;
    saveFile(outputRelockedFilename, JSON.stringify(curPackage, null, '  '));
}
